...
void core1_entry() {
    ...
    uint8_t displayBuf[80*160*2];
	  for (double y = 0, j=0; y < 160; y++,j+=0.5) {
	    for (double x = 0, k = 0; x < 80; x++, k+=0.5) {
  ...
  for(int i=0;i<80;i++)
  {
    for(int j=0;j<40;j++)
    {
      ...
      {
        largest=temp[i][j];
        largrowloc=i*2;  //row location of largest element
        largcolumnloc=j*2;   //column location of largest element
      }
    ...
    ST7735_FillRectangle(largcolumnloc, largrowloc, 5, 5,
                          0b0000011111100000);
